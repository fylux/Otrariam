...
function checkboxDestruir()
{
	if (document.getElementById("onagros").value>0)
	{
		document.getElementById("datos_asedio").style.display="block";
	}
	else
	{
		document.getElementById("datos_asedio").style.display="none";
	}
}

function edificiosDestruir()
{
	if (document.getElementById("edificio").style.display=="none")
...
<form id="enviar_tropas" name="form_mover_tropas" method="post" action="procesa_tropas.php">
	...
	<tr>
		...
		<td><input type="text" name="tropa_8" value="0" id="onagros" onKeyUp="checkboxDestruir();"/></td>
	...
	<div id="datos_asedio" style="display:none;">
		<div>
 			¿Destruir edificios?<input type="checkbox" name="destruir" onclick="edificiosDestruir();">
		</div>
		<select name="edificio" id="edificio" style="display:none;">
			<option value="ayuntamiento">Foro</option>
			...
			<option value="escondite">Escondite</option>
		</select>
	</div>
	<?php
