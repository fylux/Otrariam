...
class Mensajeria
{
	...
	public function mostrar_mensajes() //Muestra los mensajes
	{
		...
		$sql="select * from mensajes where id_destinatario=$this->id_usuario and eliminado_destinatario=0 order by id_mensaje desc";
	...
	public function mostrar_mensaje()
	{
		...
		$sql="select * from mensajes where id_mensaje=$id_mensaje and id_destinatario=$this->id_usuario";
		$res=$this->mysqli->query($sql);
		if ($res->num_rows!=0)
		{
	...
	public function enviar_mensaje($destinatario,$asunto,$mensaje,$redirect="mensajeria.php?m=1") //Envia un mensaje
	{
		$id_destinatario=safen(Datos::id($destinatario));	//ID del usuario al que va dirigido el mensaje
		...
		$asunto=safeh($asunto);	//Asunto del mensaje
		$mensaje="$this->usuario escribi&oacute;:<br />".safeh($mensaje); //Contenido del mensaje
