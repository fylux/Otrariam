...
class MotorTropas
{
		...
		for ($j=0;$j<count($movimientos);$j++)
		{
			...
			if (!isset($procesar))
			{
				if ($tiempo_restante>0)
				{
					...
					<?php
					if ($movimientos[$j][2]=='atacar' and $reg['id_ciudad_atacante'] == $id and ($reg['objetivo']=='atacar' or $reg['objetivo']=='atracar' or $reg['objetivo']=='fundar'))
					{
	...
	public function accion_tropas($objetivo,$id,$procesar=null)
	{
		...
		else if ($objetivo == 'atacar')
		{
			...
			if (!isset($_POST['destruir'])) //Si los onagros van a luchar
			{
				$ataque_tropa8=$ataque_tropa8*2.5;
			}
			...
			if ($ataque > $defensa) //Si gana el atacante
			{
				...
				if ($relacion_capacidad<=0) $relacion_capacidad=1;
				...
				//*************************************************************************
				//Buscamos el tesoro
				$sql="select * from tesoros where id_ciudad=".$reg['id_ciudad_atacada'];
				$res=$this->mysqli->query($sql);
				if ($res->num_rows>0)
				{
					$ret=$res->fetch_array();
					$sql="update tesoros set id_ciudad=".$reg['id_ciudad_atacante']." where id_ciudad=".$reg['id_ciudad_atacada']." limit 1";
					$res=$this->mysqli->query($sql);
					$sql="insert into reportes_tropas values (null,'atacar','$botin',".$reg['id_ciudad_atacante'].",
					".$reg['id_ciudad_atacada'].",'$tropas','$tropas_eliminadas','$tropasd','$tropasd',
					'tesoro|".$ret['nombre']."',0,0,".($reg['fecha']+$tiempo_tardara).")";
					$res=$this->mysqli->query($sql);
				}

				else
				{
					//Destruimos edificios si es necesario
					$sql="select * from edificios_aldea where  
					id_ciudad=".$reg['id_ciudad_atacada']." and nivel>0";
					$res=$this->mysqli->query($sql);
					if ($res->num_rows>0)
					{
						if ($reg['destruir']!='no')
						{
							$edificio =$reg['destruir'];
							$nOnagros=$tropas_restantes[7];
							$sql="select * from edificios_aldea where edificio='$edificio' 
							and id_ciudad = ".$reg['id_ciudad_atacada']." and nivel>0";
							$res=$this->mysqli->query($sql);
							if ($res->num_rows>0)
							{
								$red=$res->fetch_array();
							...
							{
								$sql="select * from edificios_aldea where  
								id_ciudad=".$reg['id_ciudad_atacada']." and nivel>0 order by rand() limit 1";
								$res=$this->mysqli->query($sql);
								$red=$res->fetch_array();
							}
							...
							for($i=1;$i<=$red['nivel'];$i++)
							{
								$defensaEdificio+=$i;
							}

							$defensaRestante=$defensaEdificio-$nOnagros;
							if ($defensaRestante<0)
							{
								$defensaRestante=0;

								//Si es un edificio de produccionEdificio
								if ($red['edificio']=='leñador' || $red['edificio']=='ladrillar' ||
								 $red['edificio']=='mina' || $red['edificio']=='granja')
								{
									$sql="update edificios_aldea set nivel=0,produccion=3, habitantes=2, slot=0 
									where id_ciudad=".$reg['id_ciudad_atacada']." and edificio='".$red['edificio']."'";
									$res=$this->mysqli->query($sql);
									$sql="update mapa set habitantes=habitantes-".($red['habitantes']+2)." where 
									id_casilla=".$reg['id_ciudad_atacada'];
									$res=$this->mysqli->query($sql);
									$sql="insert into reportes_tropas values (null,'atacar','$botin',".$reg['id_ciudad_atacante'].",
									".$reg['id_ciudad_atacada'].",'$tropas','$tropas_eliminadas','$tropasd','$tropasd',
									'asedio|".$red['edificio']."_".$red['nivel'].":0',0,0,".($reg['fecha']+$tiempo_tardara).")";
									$res=$this->mysqli->query($sql);
								}
								else
								{
									$sql="update edificios_aldea set nivel=0,produccion=0, habitantes=0, slot=0 
									where id_ciudad=".$reg['id_ciudad_atacada']." and edificio='".$red['edificio']."'";
									$res=$this->mysqli->query($sql);
									$sql="update mapa set habitantes=habitantes-".$red['habitantes']." where 
									id_casilla=".$reg['id_ciudad_atacada'];
									$res=$this->mysqli->query($sql);
									$sql="insert into reportes_tropas values (null,'atacar','$botin',".$reg['id_ciudad_atacante'].",
									".$reg['id_ciudad_atacada'].",'$tropas','$tropas_eliminadas','$tropasd','$tropasd',
							...
							else
							{
								...
								{
									$tempF+=$i;
									if ($defensaRestante<=$tempF)
									{
										$nivelRestante=$i;
										$i=100;
									}
								}
								$sql="select * from costes_construcciones where edificio='".$red['edificio']."' and nivel=".$red['nivel'];
								$res=$this->mysqli->query($sql);
								$rem=$res->fetch_array();
								$sql="select * from costes_construcciones where edificio='".$red['edificio']."' and nivel=$nivelRestante";
								$res=$this->mysqli->query($sql);
								$ret=$res->fetch_array();
								$sql="update edificios_aldea set nivel=$nivelRestante,produccion=".$ret['produccion'].", habitantes=".$ret['habitantes']." 
								where id_ciudad=".$reg['id_ciudad_atacada']." and edificio='".$ret['edificio']."'";
								$res=$this->mysqli->query($sql);
								$sql="update mapa set habitantes=habitantes-".($rem['habitantes']-$ret['habitantes'])." where 
								id_casilla=".$reg['id_ciudad_atacada'];
								$res=$this->mysqli->query($sql);
								$sql="delete from eventos where id_ciudad=".$reg['id_ciudad_atacada'];
								$res=$this->mysqli->query($sql);
								$sql="insert into reportes_tropas values (null,'atacar','$botin',".$reg['id_ciudad_atacante'].",
								".$reg['id_ciudad_atacada'].",'$tropas','$tropas_eliminadas','$tropasd','$tropasd',
								'asedio|".$red['edificio']."_".$red['nivel'].":$nivelRestante',0,0,".($reg['fecha']+$tiempo_tardara).")";
								$res=$this->mysqli->query($sql);
							}
							/*echo "<br />Numero de Onagros: ".$nOnagros."
								<br /> Edificio: ".$edificio."<br />
								Defensa Edificio: $defensaEdificio <br />
								Defensa Restante: $defensaRestante<br />
								nivel: $nivelRestante";
								exit;*/
						}
						else
		...
		else if ($objetivo == 'fundar')
		{
			$sql="select * from ataques where id_ataque=$id";
			$res=$this->mysqli->query($sql);
			$reg=$res->fetch_array();
			$id_ciudad=$reg['id_ciudad_atacada'];
			$id_usuario=Datos::propietario($reg['id_ciudad_atacante']);
			$usuario=Datos::usuario($id_usuario);
			$sql="select * from mapa where id_usuario=$id_usuario";
			$res=$this->mysqli->query($sql);
			$numero=$res->num_rows;
			$sql="update mapa set nombre = 'Pueblo de $usuario".$numero."',tipo = 'Pueblo', 
			id_usuario = $id_usuario, habitantes = 8, madera = 500,barro=500,hierro=500,cereal=500,capital = 'no',
			 last_update = $this->t_actual  where id_casilla=$id_ciudad";
			$res=$this->mysqli->query($sql);
			//************************
			...
			$sql="delete from planos where id_ciudad=".$reg['id_ciudad_atacante'];
			$res=$this->mysqli->query($sql);
		}
	}
